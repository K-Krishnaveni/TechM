...
function Divisible(props){
    const num = props.input;
        return(
            ...
            (num%13==0) && 
                ...
                <h1>{num} is not divisible by 13</h1>
            }
    </>);
}
    ...
    function StringLen(props){
        const str = props.word;
            ...
            {
             (str.length % 2 == 0)&&
            <h2>The word {str} has Even length</h2>
    }{
    (str.length % 2 != 0)&& (<h2>The word {str} has Odd Length</h2>)
    }
    </>);
    }
    ...
    function ArrayLen(props){
        const arr=props.inp;
            ...
            {
                (arr.length % 3 == 0)&&
            <h3>The array lenth [{arr.join(', ')} ]is multiple of three</h3>
            }
            {
                (arr.length % 3 != 0 )&&
        <h3>The array length [{arr.join(', ')}] is not the multiple of 3</h3>
    }
    </>);
    }
    ...
    <h4>The string {props.str}doesn't contain a vowel</h4>
      }
      </>);}
